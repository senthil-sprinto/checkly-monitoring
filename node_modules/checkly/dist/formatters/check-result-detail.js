...
export function formatResultDetailWithNavigation(result, format, hints, extraSections = []) {
...
export function groupAttemptsBySequence(results, sequenceId) {
    return results
...
export function extractResultErrorSummary(result) {
...
function firstErrorMessage(result) {
    ...
    if (browserErr)
        return formatErrorEntry(browserErr);
    const multiStepErr = result.multiStepCheckResult?.errors?.find(Boolean);
    if (multiStepErr)
        return formatErrorEntry(multiStepErr);
...
export function formatAttemptsSection(attempts, format, context = {}) {
    if (attempts.length === 0)
        return '';
    const rows = attempts.map((result, i) => ({
        result,
        runNumber: i + 1,
...
function buildAttemptColumns(format) {
    if (format === 'md') {
        return [
...
export function formatResultDetail(result, format) {
    const parts = [renderDetailFields(result.name, resultDetailFields, result, format)];
...
function colorStatus(code) {
    ...
    if (code >= 300 && code < 400)
        return chalk.yellow(String(code));
...
function appendErrors(lines, errors) {
    ...
    for (const err of errors)
        lines.push(chalk.red(`  ${formatErrorEntry(err)}`));
}
...
function formatJobLogArray(log) {
    const lines = [heading('JOB LOG', 2, 'terminal')];
    const maxLines = 50;
