...
import { renderAdaptiveTable, renderDetailFields, } from './render.js';
// --- Shared helpers ---
function formatEnabled(enabled, format) {
...
// --- Upgrade path helper ---
export function formatUpgradePath(e) {
    if (e.enabled)
        return null;
    const parts = [];
    if (e.requiredPlan) {
...
// --- Locations helper ---
export function formatLocations(locations, format) {
 ...
 */
export function getEntitlementUpgradeUrl(e, checkoutUrl) {
    if (e.requiredPlan === 'CONTRACT')
        return CONTACT_SALES_URL;
    if (e.requiredPlan || e.requiredAddon)
        return checkoutUrl;
    return CONTACT_SALES_URL;
}
function upgradeLabel(e) {
    if (e.enabled)
        return '-';
 ...
 */
function highlightDisabledRows(tableStr, items) {
...
export function formatPlanHeader(plan, format, upgradeUrl) {
    const lines = [];
    if (format === 'md') {
...
export function formatPlanSummary(plan, format, upgradeUrl) {
    const metered = [];
    const enabledFlags = [];
    const disabledFlags = [];
    ...
    if (format === 'md') {
        ...
        lines.push(`${chalk.green.bold(String(enabledFlags.length))} additional features enabled, `
...
export function formatEntitlementDetail(plan, entitlement, format, upgradeUrl) {
    const lines = [];
    lines.push(formatPlanHeader(plan, format, upgradeUrl));
    lines.push('');
...
export function formatFilteredEntitlements(plan, filtered, format, upgradeUrl) {
    const lines = [];
    lines.push(formatPlanHeader(plan, format, upgradeUrl));
    lines.push('');
