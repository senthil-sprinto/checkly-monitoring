...
import { escapeMdCell, renderAdaptiveTable, } from './render.js';
export function assetSelectorValue(asset) {
    return asset.archive?.entryName ?? asset.name;
}
function buildAssetColumns(format) {
    if (format === 'md') {
        return [
...
export function formatAssetManifestEntries(assets, format) {
    return renderAdaptiveTable(buildAssetColumns(format), assets, format);
}
function plural(count, singular, pluralValue = `${singular}s`) {
    return `${count} ${count === 1 ? singular : pluralValue}`;
}
function summarizeStorage(assets) {
...
function formatTypeCounts(assets) {
...
export function formatAssetListHeader(context, assets, format) {
    const lines = [];
...
export function formatAssetListNextSteps(context, assets, format) {
    if (assets.length === 0)
        return '';
...
function insertAsset(root, asset) {
    let node = root;
...
function leafLabel(segment, assets, format) {
    if (assets.length === 0)
        return `${segment}/`;
    const labels = assets
        .map(asset => {
...
function renderTreeNode(node, format, depth = 0) {
    const lines = [];
...
export function formatAssetManifestTree(assets, format) {
...
function buildDownloadedAssetColumns() {
    return [
        {
            header: 'Type',
            width: 12,
...
function buildDownloadedAssetStatusColumns() {
    return [
        {
            header: 'Status',
            width: 10,
...
export function formatDownloadedAssets(rows) {
    if (rows.length === 0)
        return '';
    if (rows.length === 1) {
        const [row] = rows;
