...
import { formatFrequency, formatCheckType, formatMs, timeAgo, stripAnsi, resolveResultStatus, renderDetailFields, renderAdaptiveTable, renderCommandHints, } from './render.js';
export { formatFrequency, formatCheckType } from './render.js';
export function resolveStatus(check, format) {
    if (!check.activated)
        return format === 'terminal' ? chalk.dim('inactive') : 'inactive';
    if (!check.status)
        return format === 'terminal' ? chalk.dim('-') : 'unknown';
    ...
    if (failing)
        return muted ? chalk.dim('failing') : chalk.red('failing');
    if (degraded)
        return muted ? chalk.dim('degraded') : chalk.yellow('degraded');
...
export function formatSummaryBar(statuses, totalChecks, activeCheckIds) {
    ...
    const parts = [];
    if (passing > 0)
        parts.push(chalk.green(`${logSymbols.success} ${passing} passing`));
    if (degraded > 0)
        parts.push(chalk.yellow(`${logSymbols.warning} ${degraded} degraded`));
    if (failing > 0)
        parts.push(chalk.red(`${logSymbols.error} ${failing} failing`));
...
export function formatTypeBreakdown(checks, activeCheckIds) {
...
export function formatNavigationHints(pagination, activeFilters) {
    const { page, limit, total } = pagination;
    const totalPages = Math.ceil(total / limit);
    const hints = [];
    if (page < totalPages) {
...
// --- Check detail fields ---
export const checkDetailFields = [
    ...
    },
    {
        ...
        value: (c, fmt) => {
            if (fmt === 'md')
                return null;
            if (c.scriptPath)
                return `${chalk.cyan('code')} ${chalk.dim('→')} ${c.scriptPath}`;
            return chalk.dim('UI');
        },
    },
    {
...
export function formatCheckDetail(check, format) {
...
export function formatChecks(checks, format, options = {}) {
...
export function formatErrorGroups(errorGroups, format) {
