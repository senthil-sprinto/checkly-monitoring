...
export function formatRcaDetail(rca, format) {
    if (format === 'md') {
...
export function formatRcaHint(count, format) {
    if (count <= 1)
        return '';
    const more = count - 1;
...
export function formatRcaPending(info, format) {
    if (format === 'json') {
        const output = {
            id: info.rcaId,
            status: 'pending',
...
export function formatRcaCompleted(rca, format) {
    if (format === 'json') {
        return JSON.stringify(rca, null, 2);
    }
    return formatRcaDetail(rca, format);
}
export function transformErrorGroupForJson(errorGroup) {
