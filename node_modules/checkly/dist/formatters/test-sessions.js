...
import { formatCheckType, formatDate, renderCommandHints, renderDetailFields, renderAdaptiveTable, } from './render.js';
...
function formatOptionalStatus(status, format) {
    if (!status)
...
function formatErrorGroupCount(values, format) {
    if (format === 'md')
        return formatList(values, format);
    const count = values?.length ?? 0;
    if (count === 0)
        return chalk.dim('-');
    return `${count} ${count === 1 ? 'group' : 'groups'}`;
}
function formatErrorGroupSummary(session, format) {
...
function formatNullableString(value, format) {
    if (!value)
        return format === 'terminal' ? chalk.dim('-') : '-';
    return value;
}
function formatResultBucketCount(values, format) {
    const count = values?.length ?? 0;
    if (count === 0)
        return format === 'terminal' ? chalk.dim('-') : '-';
    return String(count);
}
function formatInvoker(session, format) {
    return formatNullableString(session.commitOwner ?? session.invoker?.name, format);
}
function buildTestSessionListColumns(format) {
    if (format === 'md') {
        return [
...
export function formatTestSessionsList(sessions, format) {
    return renderAdaptiveTable(buildTestSessionListColumns(format), sessions, format);
}
export function formatTestSessionsListPaginationInfo(count, nextId) {
...
export function formatTestSessionsListNavigationHints(nextId, listCommand, firstSessionId) {
    const hints = [];
    if (nextId) {
...
function buildResultColumns(results, format) {
    if (format === 'md') {
        return [
    ...
    if (showCheckId) {
        columns.push({ header: 'Result ID', minWidth: 38, maxWidth: 38, value: result => chalk.dim(result.testSessionResultId) }, { header: 'Check ID', minWidth: 8, maxWidth: 38, value: result => result.checkId ? chalk.dim(result.checkId) : chalk.dim('-') });
    }
    else {
...
export function uniqueErrorGroupIds(session) {
    return [
        ...(session.errorGroupIds ?? []),
        ...(session.results ?? []).flatMap(result => result.errorGroupIds ?? []),
    ].filter((id, index, ids) => ids.indexOf(id) === index);
}
function errorGroupReferences(session) {
    const refs = new Map();
    const add = (id, source) => {
        const sources = refs.get(id) ?? new Set();
        sources.add(source);
        refs.set(id, sources);
    };
...
function buildErrorGroupReferenceColumns(format) {
    if (format === 'md') {
        return [
...
export function formatTestSessionErrorGroupIds(session, format, options = {}) {
...
export function formatTestSessionHints(session, format) {
    const lines = [];
...
function formatRawErrorMessage(rawErrorMessage, format, options) {
    if (!rawErrorMessage)
        return null;
...
function testSessionErrorGroupDetailFields(options) {
    return [
...
export function formatTestSessionErrorGroupDetail(group, format, options = {}) {
    return renderDetailFields('Test session error group', testSessionErrorGroupDetailFields(options), group, format);
}
export function formatTestSessionDetail(session, format, options = {}) {
    const lines = [
