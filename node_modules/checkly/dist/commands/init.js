...
import { PLATFORM_TARGETS, readSkillFile, writeSkillToTarget, } from './skills/install.js';
import { detectProjectContext, runSkillInstallStep, refreshSkill, runDepsInstall, createConfig, copyChecks, loadPromptTemplate, displayStarterPrompt, makeOnCancel, successMessage, greeting, footer, agentFooter, noSkillWarning, existingProjectFooter, } from '../helpers/onboarding/index.js';
...
export default class Init extends BaseCommand {
    ...
    async run() {
        ...
        const explicitSkillState = await this.installExplicitTargetSkill(flags.target, log);
        let context = detectProjectContext(projectDir);
        context = await this.ensureProjectPackageJson(projectDir, context, cliMode, log) ?? context;
        if (!context.isExistingProject) {
            return;
        }
        if (cliMode === 'agent') {
            await this.runAgentMode(projectDir, context, explicitSkillState);
            return;
        }
        if (cliMode === 'ci') {
            await this.runCiMode(projectDir, context, flags.target !== undefined, log);
            return;
        }
        log(greeting(this.config.version));
        if (context.hasChecklyConfig) {
            await this.runExistingProjectFlow(context, log, explicitSkillState.result);
        }
        else {
            await this.runNewProjectFlow(projectDir, context, log, explicitSkillState.result);
        }
    }
    // ─── NEW PROJECT ────────────────────────────────────────────
    async runNewProjectFlow(projectDir, context, log, explicitSkillResult) {
        const { wantAgent } = await prompts({
            type: 'confirm',
            name: 'wantAgent',
            message: 'Do you want your AI agent to set up Checkly?',
            initial: true,
        }, { onCancel: makeOnCancel(log) });
        if (wantAgent) {
            await this.runAIPath(projectDir, context, log, explicitSkillResult);
        }
        else {
            await this.runManualPath(projectDir, context, log);
        }
    }
    async runAIPath(projectDir, context, log, explicitSkillResult) {
        const skillResult = explicitSkillResult
            ?? await runSkillInstallStep(log);
        if (!skillResult.installed) {
            log(noSkillWarning());
        }
        const setupResult = await this.ensureProjectSetup(projectDir, context, log);
        if (!setupResult.ok) {
            return;
        }
        ...
        log(agentFooter(skillResult.platform, context.hasPlaywrightConfig, copiedPrompt));
    }
    async runManualPath(projectDir, context, log) {
        const setupResult = await this.ensureProjectSetup(projectDir, context, log);
        if (!setupResult.ok) {
            return;
        }
    ...
    // ─── EXISTING PROJECT ───────────────────────────────────────
    async runExistingProjectFlow(context, log, explicitSkillResult) {
        ...
        if (context.hasSkillInstalled) {
            log(chalk.dim('  Updating skill to the latest version...'));
            const result = await refreshSkill(context.skillPath, log);
            if (result.installed) {
                log(successMessage(`Skill updated at ${result.targetPath}`));
            }
        }
        else {
            await runSkillInstallStep(log);
    ...
    // ─── HELPERS ────────────────────────────────────────────────
    loadStarterPrompt(projectDir, context) {
    ...
    async installExplicitTargetSkill(target, log) {
        if (!target) {
        ...
        try {
            ...
            log(chalk.red(`Could not install skill: ${message}`));
            return { error: message, result: null };
        }
    }
    async ensureProjectPackageJson(projectDir, context, cliMode, log) {
        if (context.isExistingProject) {
            return context;
        }
    ...
    async createPackageJson(projectDir, log) {
    ...
    async ensureProjectSetup(projectDir, context, log, skipPrompts = false) {
        if (context.hasChecklyConfig) {
        ...
        const depsResult = await runDepsInstall(projectDir, log, skipPrompts ? { skipPrompts: true } : {});
    ...
    async runAgentMode(projectDir, context, explicitSkillState) {
        try {
            const noop = () => { };
            ...
            const errors = explicitSkillState.error
                ...
                : [];
            const setupResult = await this.ensureProjectSetup(projectDir, context, noop, true);
            ...
            this.log(JSON.stringify({
                ...
                playwrightConfigDetected: setupResult.context.hasPlaywrightConfig,
                playwrightConfigPath: setupResult.context.playwrightConfigPath,
    ...
    async runCiMode(projectDir, context, hasTargetFlag, log) {
        const setupResult = await this.ensureProjectSetup(projectDir, context, log, true);
        if (!setupResult.ok) {
            return;
        }
        if (!hasTargetFlag) {
