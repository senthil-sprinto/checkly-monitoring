...
import { stripAnsi, formatDate, renderCommandHints, } from '../../formatters/render.js';
import { formatCheckDetail, formatResults, formatErrorGroups, } from '../../formatters/checks.js';
import { formatResultDetailWithNavigation, formatAttemptsSection, groupAttemptsBySequence, } from '../../formatters/check-result-detail.js';
import { formatRcaDetail, formatRcaHint, transformErrorGroupForJson } from '../../formatters/rca.js';
import { quickRangeValues } from '../../rest/analytics.js';
...
export default class ChecksGet extends AuthCommand {
        ...
        try {
            ...
            const [statusResp, resultsResp, errorGroupsResp, analyticsResp] = await Promise.all([
                ...
                api.analytics.get(args.id, check.checkType, {
                    quickRange: (flags['stats-range'] ?? 'last24Hours'),
                    groupBy: flags['group-by'] === 'location' ? 'runLocation' : flags['group-by'],
                    metrics: flags.metrics ? flags.metrics.split(',').map(m => m.trim()) : undefined,
                    filterByStatus: flags['filter-status'],
    ...
    async showErrorGroupDetail(checkId, errorGroupId, outputFormat) {
        ...
        if (fmt === 'md') {
            ...
            lines.push('', `| Field | Value |`, `| --- | --- |`, `| First seen | ${formatDate(errorGroup.firstSeen, fmt)} |`, `| Last seen | ${formatDate(errorGroup.lastSeen, fmt)} |`, `| Error group | ${errorGroup.id} |`);
            if (check.scriptPath) {
    ...
    async showResultDetail(checkId, resultId, outputFormat, includeAttempts) {
    ...
    renderAttemptsSection(attempts, resultId, fmt) {
    ...
    static MAX_SEQUENCE_SPAN_SECONDS = 30 * 60;
    ...
    async fetchAttempts(checkId, result) {
