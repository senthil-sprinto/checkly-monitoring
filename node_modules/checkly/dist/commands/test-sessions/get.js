...
import { formatTestSessionDetail, formatTestSessionErrorGroupDetail, uniqueErrorGroupIds, } from '../../formatters/test-sessions.js';
...
export default class TestSessionsGet extends AuthCommand {
        ...
        try {
            ...
            if (flags['error-group']) {
                ...
                if (!errorGroupIds.includes(flags['error-group'])) {
                    ...
                    this.style.longError('Test session error group not found in this session.', `Run \`${listCommand}\` to list the available error group IDs.`);
                    process.exitCode = 1;
                    return;
                }
    ...
    async getTestSession(id, watch, showAction) {
        if (!watch) {
    ...
    async showResultDetail(testSessionId, resultId, outputFormat, watch) {
        if (watch) {
