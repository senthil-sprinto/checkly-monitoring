...
import { formatTestSessionsList, formatTestSessionsListNavigationHints, formatTestSessionsListPaginationInfo, } from '../../formatters/test-sessions.js';
const statusOptions = ['running', 'failed', 'passed', 'cancelled'];
const providerOptions = ['github', 'vercel', 'api', 'trigger', 'pw_reporter'];
function quoteArg(value) {
    if (/^[A-Za-z0-9._/@:+=,-]+$/.test(value))
        return value;
    return `'${value.replace(/'/g, `'\\''`)}'`;
}
function parseUnixTimestamp(value, label) {
    if (!value)
        return undefined;
...
function buildListCommand(flags) {
    ...
    if (flags.from)
        parts.push('--from', quoteArg(flags.from));
    if (flags.to)
        parts.push('--to', quoteArg(flags.to));
    ...
    if (flags['no-users'])
        parts.push('--no-users');
    if (flags.search)
        parts.push('--search', quoteArg(flags.search));
    if (flags['error-group'])
        parts.push('--error-group', quoteArg(flags['error-group']));
    return parts.join(' ');
}
    ...
    async run() {
        ...
        try {
            ...
            if (fmt === 'terminal') {
                ...
                const navHints = formatTestSessionsListNavigationHints(data.nextId, buildListCommand(flags), data.entries[0]?.id);
                if (navHints) {
                    lines.push('');
                    lines.push(navHints);
                }
