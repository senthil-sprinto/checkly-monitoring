...
import { Check, AlertChannelSubscription, AlertChannel, CheckGroup, Dashboard, MaintenanceWindow, PrivateLocation, PrivateLocationCheckAssignment, PrivateLocationGroupAssignment, Diagnostics, Session, } from '../constructs/index.js';
...
export default class Deploy extends AuthCommand {
    ...
    async run() {
        ...
        const { force, preview, 'schedule-on-deploy': scheduleOnDeploy, output: outputFlag, verbose, config: configFilename, 'verify-runtime-dependencies': verifyRuntimeDependencies, 'debug-bundle': debugBundle, 'debug-bundle-output-file': debugBundleOutputFile, } = flags;
    ...
    formatPreview(previewData, project, verbose = false) {
        ...
        if (sortedCreating.filter(({ construct }) => Boolean(construct)).length) {
            ...
            for (const { logicalId, physicalId, construct } of sortedCreating) {
                ...
                if (verbose && construct.name) {
                    output.push(`      name: ${construct.name}`);
                }
