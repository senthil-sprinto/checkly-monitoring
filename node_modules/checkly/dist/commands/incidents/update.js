...
import { formatIncidentUpdateDetail, formatIncidentSeverity } from '../../formatters/incidents.js';
import { incidentProgressStatusOptions, toIncidentProgressStatus, incidentSeverityOptions, toIncidentSeverity, } from '../../helpers/incidents.js';
        ...
        try {
            if (flags.severity) {
                await api.incidents.update(args.id, {
                    severity: toIncidentSeverity(flags.severity),
                });
            }
            const update = await api.incidents.createUpdate(args.id, {
                description: flags.message,
                status: toIncidentProgressStatus(flags.status),
            ...
            if (fmt === 'md') {
                ...
                if (flags.severity) {
                    lines.push('');
                    lines.push(`Severity updated to **${formatIncidentSeverity(toIncidentSeverity(flags.severity), fmt)}**.`);
                }
                this.log(lines.join('\n'));
                return;
            }
            ...
            if (flags.severity) {
                output.push(`  ${chalk.dim('Severity:')}   ${formatIncidentSeverity(toIncidentSeverity(flags.severity), fmt)}`);
            }
            output.push('');
