...
import { findPlaywrightConfigPath, getCiInformation, getDefaultChecklyConfig, getEnvs, getGitInformation, splitConfigFilePath, writeChecklyConfigFile, } from '../services/util.js';
import { getChecklyConfigFile, loadChecklyConfig } from '../services/checkly-config-loader.js';
...
import { DEFAULT_PLAYWRIGHT_CHECK_RUN_TIMEOUT_SECONDS, Events, } from '../services/abstract-check-runner.js';
import commonMessages from '../messages/common-messages.js';
import path from 'node:path';
import * as recast from 'recast';
import { addItemToArray, addOrReplaceItem, findPropertyByName, reWriteChecklyConfigFile, } from '../helpers/write-config-helpers.js';
...
export default class PwTestCommand extends AuthCommand {
    ...
    async run() {
        ...
        const { location: runLocation, 'private-location': privateRunLocation, env = [], 'env-file': envFile, timeout, verbose: verboseFlag, reporter: reporterFlag, config: configFilename, record, 'test-session-name': testSessionName, 'create-check': createCheck, 'stream-logs': streamLogs, 'include': includeFlag, 'frequency': frequency, 'install-command': installCommand, 'refresh-cache': refreshCache, 'detach': detach, } = flags;
        const { configDirectory, configFilenames } = splitConfigFilePath(configFilename);
        const pwPathFlag = this.getConfigPath(playwrightFlags);
        const { config: checklyConfig, constructs: checklyConfigConstructs, } = await loadChecklyConfig(configDirectory, configFilenames, false, pwPathFlag);
        ...
        const playwrightCheck = await PwTestCommand.createPlaywrightCheck(playwrightFlags, runLocation, privateRunLocation, dir, frequency, installCommand);
        if (createCheck) {
        ...
        const runner = new TestRunner(config.getAccountId(), projectBundle, checkBundles, Session.sharedFiles, location, timeout, verboseFlag, record, repoInfo, ciInfo.environment, 
        // NO NEED TO UPLOAD SNAPSHOTS FOR PLAYWRIGHT TESTS
        false, configDirectory, 
        // TODO: ADD PROPER RETRY STRATEGY HANDLING
        null, // testRetryStrategy
        streamLogs, refreshCache, detach);
        ...
        const noTestsFoundChecks = new Set();
        ...
        runner.on(Events.STREAM_LOGS, (check, sequenceId, logs) => {
            reporters.forEach(r => r.onStreamLogs(check, sequenceId, logs));
            const hasNoTestsFound = logs.some((log) => log.message?.includes('No tests found'));
            if (hasNoTestsFound) {
    ...
    static async createPlaywrightCheck(args, runLocation, privateRunLocation, dir, frequency = 10, installCommand) {
