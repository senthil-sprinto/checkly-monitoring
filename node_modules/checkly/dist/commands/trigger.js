...
export default class Trigger extends AuthCommand {
    ...
    async run() {
        ...
        const { location: runLocation, 'private-location': privateRunLocation, config: configFilename, tags: targetTags, 'check-id': targetCheckIds, timeout, verbose: verboseFlag, 'fail-on-no-matching': failOnNoMatching, record: shouldRecord, reporter: reporterFlag, env, 'env-file': envFile, 'test-session-name': testSessionName, retries, 'refresh-cache': refreshCache, 'detach': detach, } = flags;
        ...
        const reporterTypes = this.prepareReportersTypes(reporterFlag, checklyConfig?.cli?.reporters);
        if (detach) {
        ...
        const runner = new TriggerRunner(config.getAccountId(), timeout, verbose, shouldRecord, location, targetTags?.map((tags) => tags.split(',')) ?? [], targetCheckIds ?? [], Object.entries(envVars).map(([key, value]) => ({ key, value })), repoInfo, ciInfo.environment, testSessionName, testRetryStrategy, refreshCache, detach);
        ...
        runner.on(Events.CHECK_SUCCESSFUL, (sequenceId, _, result, testResultId, links) => {
