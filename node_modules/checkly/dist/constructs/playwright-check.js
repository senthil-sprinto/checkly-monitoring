...
import { ConflictingPropertyDiagnostic, DeprecatedPropertyDiagnostic, InvalidPropertyValueDiagnostic, SubstitutedPropertyValueDiagnostic, UnsatisfiedLocalPrerequisitesDiagnostic, UnsupportedPropertyDiagnostic, } from './construct-diagnostics.js';
import { WarningDiagnostic } from './diagnostics.js';
...
export class PlaywrightCheck extends RuntimeCheck {
    ...
    configDefaultsGetter(props) {
        ...
        return makeConfigDefaultsGetter(group?.getCheckDefaults(), {
    ...
    async validateDoubleCheck(diagnostics) {
        if (this.doubleCheck !== undefined) {
            diagnostics.add(new UnsupportedPropertyDiagnostic('doubleCheck', new Error(`This property is not available in Playwright checks.`
                + `\n\n`
    ...
    async validateRetryStrategy(diagnostics) {
        ...
        if (this.retryStrategy) {
            diagnostics.add(new UnsupportedPropertyDiagnostic('retryStrategy', new Error(`This property is not available in Playwright checks.`
                + `\n\n`
                + `Playwright tests have their own built-in retry mechanism that `
                + `should be configured in your Playwright configuration file `
                + `instead.`)));
    ...
    async validateHeadlessMode(diagnostics) {
        try {
            ...
            if (playwrightConfig?.use?.headless === false) {
                diagnostics.add(new InvalidPropertyValueDiagnostic('headless', new Error(`headless: false is not supported.`
                    + `\n\n`
            ...
            if (Array.isArray(playwrightConfig?.projects)) {
                for (const project of playwrightConfig.projects) {
                    if (project?.use?.headless === false) {
                        ...
                        diagnostics.add(new InvalidPropertyValueDiagnostic('headless', new Error(`headless: false is not supported${projectName}.`
                            + `\n\n`
                            + `Checkly runs all Playwright checks in headless mode. `
                            + `Please remove this setting from your Playwright configuration, `
                            + `or set it to true.`)));
            ...
            diagnostics.add(new InvalidPropertyValueDiagnostic('playwrightConfigPath', new Error(`Unable to parse Playwright config "${this.playwrightConfigPath}": ${err.message}`, { cause: err })));
        }
    }
    ...
    async validate(diagnostics) {
        ...
        try {
            ...
            diagnostics.add(new InvalidPropertyValueDiagnostic('playwrightConfigPath', new Error(`Playwright config "${this.playwrightConfigPath}" does not exist: ${err.message}`, { cause: err })));
        }
    ...
    async #validateWorkspace(diagnostics) {
        ...
        if (workspace.isOk()) {
            ...
            if (lockfile.isErr()) {
                diagnostics.add(new UnsatisfiedLocalPrerequisitesDiagnostic(new Error(`A lockfile is required for Playwright checks, but none could be `
                    + `detected.`
                    + '\n\n'
            ...
            diagnostics.add(new UnsatisfiedLocalPrerequisitesDiagnostic(new Error(`A workspace is required for Playwright checks, but none could be `
                + `detected.`
                + '\n\n'
    ...
    #validateGroupReferences(diagnostics) {
        if (this.groupName) {
            diagnostics.add(new DeprecatedPropertyDiagnostic('groupName', new Error(`Use the "group" property instead. Depending on load order, `
                + 'group defaults may not work correctly when using "groupName".')));
            if (this.groupId) {
                diagnostics.add(new ConflictingPropertyDiagnostic('groupName', 'group', new Error(`Prefer the "group" property over "groupName".`)));
            }
            const checkGroup = _a.#findGroupByName(this.groupName);
            if (!checkGroup) {
                diagnostics.add(new InvalidPropertyValueDiagnostic('groupName', new Error(`No such group "${this.groupName}".`)));
            }
        }
    }
    getSourceFile() {
    ...
    async bundle(bundler) {
        ...
        const groupId = this.groupName && !this.groupId
            ...
            : this.groupId;
        const { browsers, playwrightVersion, relativePlaywrightConfigPath, workingDir, files, } = await Session.getPlaywrightProjectBundler().bundle(this.playwrightConfigPath, this.include ?? []);
