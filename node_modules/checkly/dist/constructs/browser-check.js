...
export class BrowserCheck extends RuntimeCheck {
    ...
    configDefaultsGetter(props) {
        return makeConfigDefaultsGetter(props.group?.getBrowserCheckDefaults(), Session.browserCheckDefaults, props.group?.getCheckDefaults(), Session.checkDefaults);
    }
    ...
    async bundle(bundler) {
        return new BrowserCheckBundle(this, await (async () => {
            if (isEntrypoint(this.code)) {
                const bundle = await BrowserCheck.bundle(bundler, this.resolveContentFilePath(this.code.entrypoint), this.runtimeId);
                if (!bundle.script) {
