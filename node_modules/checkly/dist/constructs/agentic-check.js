...
export class AgenticCheck extends Check {
    ...
    async validate(diagnostics) {
        ...
        if (!this.prompt || this.prompt.trim().length === 0) {
            diagnostics.add(new InvalidPropertyValueDiagnostic('prompt', new Error('"prompt" is required and must not be empty.')));
        }
        else if (this.prompt.length > 10_000) {
            diagnostics.add(new InvalidPropertyValueDiagnostic('prompt', new Error(`"prompt" must be at most 10000 characters, got ${this.prompt.length}.`)));
        }
        ...
        if (this.agentRuntime?.skills) {
            for (const [index, skill] of this.agentRuntime.skills.entries()) {
                if (typeof skill !== 'string' || skill.trim().length === 0) {
                    diagnostics.add(new InvalidPropertyValueDiagnostic('agentRuntime.skills', new Error(`"agentRuntime.skills[${index}]" must be a non-empty string.`)));
                }
            }
        }
    }
