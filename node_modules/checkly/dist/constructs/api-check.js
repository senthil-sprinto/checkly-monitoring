...
import { ApiCheckBundle } from './api-check-bundle.js';
...
export class ApiCheck extends RuntimeCheck {
    ...
    async validate(diagnostics) {
        ...
        if (this.setupScript) {
            if (!isEntrypoint(this.setupScript) && !isContent(this.setupScript)) {
                diagnostics.add(new InvalidPropertyValueDiagnostic('setupScript', new Error(`Either "entrypoint" or "content" is required.`)));
            }
            else if (isEntrypoint(this.setupScript) && isContent(this.setupScript)) {
                diagnostics.add(new InvalidPropertyValueDiagnostic('setupScript', new Error(`Provide exactly one of "entrypoint" or "content", but not both.`)));
            }
                ...
                try {
                    ...
                    diagnostics.add(new InvalidPropertyValueDiagnostic('setupScript', new Error(`Unable to access entrypoint file "${entrypoint}": ${err.message}`, { cause: err })));
                }
            }
        }
        if (this.localSetupScript) {
            diagnostics.add(new DeprecatedPropertyDiagnostic('localSetupScript', new Error(`Use "setupScript" instead.`)));
        }
        if (this.tearDownScript) {
            if (!isEntrypoint(this.tearDownScript) && !isContent(this.tearDownScript)) {
                diagnostics.add(new InvalidPropertyValueDiagnostic('tearDownScript', new Error(`Either "entrypoint" or "content" is required.`)));
            }
            else if (isEntrypoint(this.tearDownScript) && isContent(this.tearDownScript)) {
                diagnostics.add(new InvalidPropertyValueDiagnostic('tearDownScript', new Error(`Provide exactly one of "entrypoint" or "content", but not both.`)));
            }
                ...
                try {
                    ...
                    diagnostics.add(new InvalidPropertyValueDiagnostic('tearDownScript', new Error(`Unable to access entrypoint file "${entrypoint}": ${err.message}`, { cause: err })));
                }
            }
        }
        if (this.localTearDownScript) {
            diagnostics.add(new DeprecatedPropertyDiagnostic('localTearDownScript', new Error(`Use "tearDownScript" instead.`)));
        }
        if (this.runtimeId) {
            const runtime = Session.getRuntime(this.runtimeId);
            if (!runtime) {
                diagnostics.add(new InvalidPropertyValueDiagnostic('runtimeId', new Error(`"${this.runtimeId}" is not a known runtime.`)));
            }
        }
    ...
    async bundle(bundler) {
        ...
        if (this.setupScript) {
            if (isEntrypoint(this.setupScript)) {
                const { script, scriptPath, dependencies } = await ApiCheck.bundle(bundler, this.resolveContentFilePath(this.setupScript.entrypoint), this.runtimeId);
        ...
        if (this.tearDownScript) {
            if (isEntrypoint(this.tearDownScript)) {
                const { script, scriptPath, dependencies } = await ApiCheck.bundle(bundler, this.resolveContentFilePath(this.tearDownScript.entrypoint), this.runtimeId);
    ...
    static async bundle(bundler, entrypoint, runtimeId) {
