...
function isFrequencyLike(value) {
    if (typeof value !== 'object' || value === null) {
        return false;
    }
    const maybeFrequency = value;
...
export class Check extends Construct {
    name;
    description;
    activated;
    muted;
            ...
            if (retryStrategy.onlyOn) {
                if (retryStrategy.onlyOn === 'NETWORK_ERROR') {
                    if (!this.supportsOnlyOnNetworkErrorRetryStrategy()) {
                        diagnostics.add(new InvalidPropertyValueDiagnostic('retryStrategy', new Error(`Using "NETWORK_ERROR" with "onlyOn" is only supported in the `
                            + `ApiCheck and UrlMonitor constructs.`)));
                    }
                }
                else {
                    diagnostics.add(new InvalidPropertyValueDiagnostic('retryStrategy', new Error(`Unsupported value "${retryStrategy.onlyOn}" for "onlyOn".`)));
                }
            }
        }
    }
    ...
    configDefaultsGetter(props) {
        return makeConfigDefaultsGetter(props.group?.getCheckDefaults(), Session.checkDefaults);
    }
...
export class RuntimeCheck extends Check {
    runtimeId;
