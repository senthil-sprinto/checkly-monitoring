...
export function greeting(version) {
...
export function footer(hasPlaywright = false) {
    const lines = [
        '',
...
export function agentFooter(platform, hasPlaywright = false, copiedPrompt = false) {
    ...
    lines.push('', chalk.dim('  Your agent will use the skill to create a'), chalk.dim('  tailored Checkly setup based on your project.'));
    if (hasPlaywright) {
        lines.push(...playwrightBlockDimmed());
    }
    lines.push('', chalk.dim('  Prefer to set up manually?'), chalk.dim(`  ${chalk.bold('npx checkly login')}`), chalk.dim(`  ${chalk.bold('npx checkly test')}`), chalk.dim(`  ${chalk.bold('npx checkly deploy')}`));
    lines.push(...docsBlock());
    return lines.join('\n');
}
export function noSkillWarning() {
    return [
        '',
        chalk.yellow(wrap('Note: without the skill, your agent won\'t have Checkly-specific knowledge.', { length: WARNING_WRAP_WIDTH, prefix: '  ' })),
        '',
...
export function existingProjectFooter(hasPlaywright = false) {
    const lines = [
        '',
