...
export function createConfig(projectDir, log) {
...
export function copyChecks(projectDir, log) {
...
function installDependencies(projectDir, installCmd, log) {
    try {
        ...
        log(chalk.red(`Failed to install dependencies. Run ${chalk.bold(installCmd)} manually. ${error.message?.slice(0, 200) ?? ''}`));
        return {
            ok: false,
            packageJsonUpdated: true,
            installed: false,
...
export async function runDepsInstall(projectDir, log, options = {}) {
