...
const unsupportedDetachedReporterTypes = ['json', 'github'];
export async function prepareRunLocation(configOptions = {}, cliFlags = {}, api, accountId) {
...
export async function preparePrivateRunLocation(privateLocationSlugName, api, accountId) {
    try {
...
export function prepareReportersTypes(reporterFlag, cliReporters = []) {
    if (!reporterFlag?.length && !cliReporters.length) {
        return [isCI ? 'ci' : 'list'];
    }
    return reporterFlag?.length ? reporterFlag : cliReporters;
}
export function validateDetachReporterTypes(reporterTypes) {
    const unsupported = [...new Set(reporterTypes.filter(type => unsupportedDetachedReporterTypes.includes(type)))];
    if (unsupported.length) {
        throw new Error(`--detach does not support ${unsupported.map(type => `--reporter ${type}`).join(', ')}. `
...
export function splitChecklyAndPlaywrightFlags(args) {
