...
export default class ChecklyHelpClass extends Help {
    ...
    showRootHelp() {
        ...
        this.log(this.section('ENVIRONMENT VARIABLES', `The CLI automatically loads variables from a .env file in the current directory.
  To disable, set CHECKLY_NO_DOTENV=1.

  CHECKLY_ACCOUNT_ID    Checkly account ID.
  CHECKLY_API_KEY       Checkly User API Key.
    `));
        const skillsCommands = this.renderList(skills.map(s => [s.command, s.description]), {
            spacer: '\n',
            stripAnsi: this.opts.stripAnsi,
            indentation: 2,
        });
