...
import { findFilesWithPattern, getPlaywrightConfigPath, pathToPosix, } from './util.js';
import { Check, BrowserCheck, Project, Session, PrivateLocation, PrivateLocationCheckAssignment, PrivateLocationGroupAssignment, MultiStepCheck, } from '../constructs/index.js';
...
import { detectNearestPackageJson, detectPackageManager, fauxWorkspaceFromPackageJson, NoPackageJsonFoundError, } from './check-parser/package-files/package-manager.js';
...
async function findBasePath(packageManager, directory, { ignoreWorkspaces = false }) {
    try {
        if (ignoreWorkspaces) {
            const nearestPackageJson = await detectNearestPackageJson(directory);
            const workspace = await fauxWorkspaceFromPackageJson(packageManager, nearestPackageJson);
            return {
...
export async function parseProject(opts) {
    ...
    filteredConstructs?.forEach(construct => project.addResource(construct.type, construct.logicalId, construct));
