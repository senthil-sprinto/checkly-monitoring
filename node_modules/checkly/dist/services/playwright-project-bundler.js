...
export class PlaywrightProjectBundler {
    ...
    #cache = new Map();
    async bundle(playwrightConfig, include) {
    ...
    async bundleProject(playwrightConfig, include) {
        ...
        function includeTargets(dirName) {
        ...
        const includedFiles = await findFilesWithPattern(dir, effectiveIncludes, ignoredFiles);
...
export function normalizeVersion(v) {
    const cleaned = semver.valid(semver.clean(v ?? '') || '')
 ...
 */
export async function resolvePlaywrightVersion(cwd) {
...
function playwrightRange(packageJson) {
 ...
 * fall back.
 */
async function getPlaywrightVersionFromLockfile(cwd) {
    ...
    if (consumingRange !== undefined) {
        ...
        if (validRange && !semver.satisfies(version, validRange)) {
            process.stderr.write(`Warning: resolved @playwright/test version ${version} does not satisfy the range `
...
function toImporterRelPath(rootPath, packagePath) {
...
export async function getPlaywrightVersionFromPackage(cwd) {
    try {
...
//# sourceMappingURL=playwright-project-bundler.js.map