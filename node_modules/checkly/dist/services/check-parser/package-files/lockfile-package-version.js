 ...
 */
export function parseNpmLockfileVersion(content, query) {
 ...
 */
export function parsePnpmLockfileVersion(content, query) {
 ...
 */
export function parseBunLockfileVersion(content, query) {
    const data = JSON5.parse(content);
    const packages = data?.packages;
    if (typeof packages !== 'object' || packages === null) {
        return undefined;
 ...
 */
function pickYarnVersion(resolutions, declaredRange) {
    ...
    if (declaredRange !== undefined) {
        ...
        if (satisfying.length > 0) {
            return satisfying.reduce((best, candidate) => semver.gt(candidate.version, best.version) ? candidate : best).version;
        }
    }
 ...
 */
function resolveYarnVersion(resolutions, query) {
    for (const candidate of query.importers) {
 ...
 */
function parseYarnBerryResolutions(content, packageName) {
    const data = parseYaml(content);
    ...
    for (const [key, value] of Object.entries(data)) {
        ...
        const ranges = key
            ...
            .filter((parsed) => parsed !== undefined && parsed.name === packageName)
 ...
 */
function parseYarnClassicResolutions(content, packageName) {
    const resolutions = [];
 ...
 */
export function parseYarnLockfileVersion(content, query) {
...
//# sourceMappingURL=lockfile-package-version.js.map