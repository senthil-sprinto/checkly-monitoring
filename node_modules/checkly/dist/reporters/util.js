...
(function (CheckStatus) {
    ...
    CheckStatus[CheckStatus["CANCELLED"] = 6] = "CANCELLED";
...
export function formatCheckTitle(status, check, opts = {}) {
    let duration;
...
export function formatCheckResult(checkResult) {
    const result = [];
    ...
    if (checkResult.checkType === 'AGENTIC' && checkResult.agenticCheckResult) {
        ...
        if (Array.isArray(agentic.assertions) && agentic.assertions.length > 0) {
            const assertionLines = agentic.assertions.map((a) => {
        ...
        if (Array.isArray(agentic.suggestions) && agentic.suggestions.length > 0) {
            const suggestionLines = agentic.suggestions.map((s) => {
...
function formatAssertions(assertions, options) {
...
function formatDnsRequest(request) {
    return [
        `Query: ${request.query}`,
...
function formatDnsNameServerInfo(nameServerInfo) {
    return [
...
function formatIcmpRequest(request) {
    return [
...
function formatIcmpResponse(response) {
    return [
...
export function resultToCheckStatus(checkResult) {
...
export function printLn(text, afterLnCount = 1, beforeLnCount = 0) {
    process.stdout.write(`${'\n'.repeat(beforeLnCount)}${text}${'\n'.repeat(afterLnCount)}`);
}
export function getTestSessionUrl(testSessionId) {
    const { baseURL, accountId } = getDefaults();
    return `${baseURL.replace(/api/, 'app')}/accounts/${accountId}/test-sessions/${testSessionId}`;
}
export function getTraceUrl(traceUrl) {
